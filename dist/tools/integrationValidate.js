...
function parseIntegrationMappings(content) {
    const result = { actions: [], operations: [] };
...
export const narukanaIntegrationValidate = tool({
    description: "Deep-scan: validate integration consistency between UI and backend/contract (read-only)",
    args: {},
    execute: async (_args, ctx) => {
        ...
        try {
            ...
            const missingMappingsInIntegration = [];
            const noUiEvidence = [];
            const noBackendEvidence = [];
            const unusedOps = [];
            ...
            const hasIssues = missingMappingsInIntegration.length > 0 ||
...
//# sourceMappingURL=integrationValidate.js.map