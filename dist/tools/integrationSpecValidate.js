...
function validateIntegrationSpec(content) {
    const warnings = [];
    const errors = [];
    const required = [
        "## Runtime Flow",
    ...
    // Soft warnings
    if (!content.match(/## Mappings[\s\S]*- action:/))
        warnings.push("No '- action:' mappings found under ## Mappings");
    if (!content.match(/## Contract Operations[\s\S]*- /))
        warnings.push("No operations listed under ## Contract Operations");
...
//# sourceMappingURL=integrationSpecValidate.js.map