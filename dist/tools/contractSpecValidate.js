...
function validateContractJson(content) {
    const warnings = [];
    const errors = [];
    const operations = [];
    let data;
    ...
    if (!data.operations || typeof data.operations !== "object") {
        ...
        for (const [opName, op] of Object.entries(data.operations)) {
            operations.push(opName);
            if (!op.type)
                errors.push(`Operation '${opName}' missing 'type' field`);
            if (!op.transport)
                errors.push(`Operation '${opName}' missing 'transport' field`);
            if (op.input === undefined)
                warnings.push(`Operation '${opName}' missing 'input' field (operations may have no input)`);
            if (op.output === undefined)
                warnings.push(`Operation '${opName}' missing 'output' field (operations may have no output)`);
            if (op.transport === "http") {
                if (!op.method)
                    errors.push(`Operation '${opName}' is HTTP but missing 'method'`);
                if (!op.endpoint)
                    errors.push(`Operation '${opName}' is HTTP but missing 'endpoint'`);
            }
            else if (op.transport === "contract") {
                if (!op.target)
                    errors.push(`Operation '${opName}' is contract but missing 'target'`);
                if (!op.function)
                    errors.push(`Operation '${opName}' is contract but missing 'function'`);
            }
        }
        if (operations.length === 0)
            warnings.push("No operations defined in contract.json");
    }
...
//# sourceMappingURL=contractSpecValidate.js.map